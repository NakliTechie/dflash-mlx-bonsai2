...
const PORT = 9338;
const chrome = spawn(CH, ['--headless=new', '--no-first-run', '--no-default-browser-check', `--user-data-dir=${process.cwd()}/chrome-profile-runner`, '--enable-unsafe-webgpu', '--enable-features=WebGPU', `--remote-debugging-port=${PORT}`, '--window-size=1280,800', 'about:blank'], { stdio: 'ignore' });
...
let ws; try {
  ...
  while ((Date.now() - t0) / 1000 < Number(tmo)) {
    const st = await evalJs('JSON.stringify({state: window.__dr && window.__dr.state, prog: window.__dr && window.__dr.prog})');
  ...
  console.log('RESULTS', await evalJs('JSON.stringify({gpu: window.__dr.gpu, prompt: window.__dr.prompt, drafter: window.__dr.drafter, embedRoute: window.__dr.embedRoute, results: window.__dr.results, error: window.__dr.error})'));
