...
let ws; try {
  ...
  ws.onmessage = (ev) => { const m = JSON.parse(ev.data); if (m.id && pending.has(m.id)) { pending.get(m.id)(m); pending.delete(m.id); } else if (m.method === 'Runtime.consoleAPICalled') { const a = m.params.args.map(x => x.value ?? x.description).join(' '); if (/error|warn|fail/i.test(a)) console.log('[console]', a.slice(0, 300)); } };
  ...
  console.log('RESULTS', await evalJs('JSON.stringify({results: window.__vs.results, error: window.__vs.error && String(window.__vs.error).slice(0, 600), featError: window.__vs.featError, log: window.__vs.log.map(l => l[1])})'));
