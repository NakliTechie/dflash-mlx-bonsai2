...
~/venv/bin/python train_adapter_torch.py --full --gen-only --init "$INIT" --shards ~/shards_gen --epochs 2 --stride 4 --lr 2e-5 --window 160 --eval-docs 60 --out ~/adapter-r3 2>&1 | grep -E 'eval|train:|shards in|step [0-9]*000/|saved|Error|Traceback' | tee -a ~/post_capture.log
