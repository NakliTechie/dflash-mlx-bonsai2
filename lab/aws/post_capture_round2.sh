...
log "round 2: full fine-tune on general + chat shards, stride 8, 2 epochs, init $INIT"
( while true; do sleep 600; aws s3 sync ~/adapter-r2 "$S3/adapter-r2" --only-show-errors; done ) & SYNC=$!
~/venv/bin/python train_adapter_torch.py --full --init "$INIT" --shards ~/shards,~/shards_chat --epochs 2 --stride 8 --lr 2e-5 --out ~/adapter-r2 2>&1 | grep -E 'eval|train:|step [0-9]*000/|saved|Error|Traceback' | tee -a ~/post_capture.log
