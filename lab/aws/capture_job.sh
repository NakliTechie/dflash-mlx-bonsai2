...
"$BIN" -m "$MODEL" --corpus "$CORPUS" --out "$OUT" --taps 5,19,33,47,61 --budget "$BUDGET" --start-doc "$SD" --start-shard "$SS" -c 2048 -b 512 -ub 512 -ngl 99 -fa on 2>&1 | grep -E 'shard|done|doc [0-9]+/|error|failed' | tee -a ~/capture_job.log
